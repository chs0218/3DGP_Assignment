...
//============================================================================================
#define FIRST_CAR_CAMERA 0x04
#define CAR_CAMERA 0x05
#define THIRD_CAR_CAMERA 0x06

class CMyPlayer;

struct VS_CB_RACE_CAMERA_INFO
{
	XMFLOAT4X4 m_xmf4x4View;
	XMFLOAT4X4 m_xmf4x4Projection;
};
...
protected:
	...
	CMyPlayer* m_pPlayer = NULL;
public:
	CMyCamera();
	CMyCamera(CMyCamera* pCamera);
	virtual ~CMyCamera();
	...
	CMyPlayer* GetPlayer() { return(m_pPlayer); }
	void SetMode(DWORD nMode) { m_nMode = nMode; }
	DWORD GetMode() { return(m_nMode); }
	void SetPosition(XMFLOAT3 xmf3Position) { m_xmf3Position = xmf3Position; }
	XMFLOAT3& GetPosition() { return(m_xmf3Position); }
...
class CCarCamera : public CMyCamera
{
public:
	CCarCamera(CMyCamera* pCamera);
	virtual ~CCarCamera() { }
	virtual void Rotate(float fPitch = 0.0f, float fYaw = 0.0f, float fRoll = 0.0f);
};

class CFirstCarCamera : public CMyCamera
{
public:
	CFirstCarCamera(CMyCamera* pCamera);
	virtual ~CFirstCarCamera() { }
	virtual void Rotate(float fPitch = 0.0f, float fYaw = 0.0f, float fRoll = 0.0f);
};

class CThirdCarCamera : public CMyCamera
{
public:
	CThirdCarCamera(CMyCamera* pCamera);
	virtual ~CThirdCarCamera() { }
	virtual void Update(XMFLOAT3& xmf3LookAt, float fTimeElapsed);
	virtual void SetLookAt(XMFLOAT3& vLookAt);
};