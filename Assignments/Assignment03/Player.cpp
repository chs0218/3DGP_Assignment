...
void CPlayer::Update(float fTimeElapsed, CScene* myScene)
{
...
CCamera* CPlayer::ToggleCamera()
{
	toggleCamera = !toggleCamera;
	if (toggleCamera)
		return ChangeCamera(THIRD_PERSON_CAMERA, 0.0f);
	else
		return ChangeCamera(FIRST_PERSON_CAMERA, 0.0f);
...
CCamera *CAirplanePlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	...
	case FIRST_PERSON_CAMERA:
		...
		m_pCamera->SetOffset(XMFLOAT3(0.0f, 0.6f, 0.0f));
...
void CMyPlayer::CreateShaderVariables(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	if (m_pCamera) m_pCamera->CreateShaderVariables(pd3dDevice, pd3dCommandList);
}

void CMyPlayer::UpdateShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
{
}

void CMyPlayer::ReleaseShaderVariables()
{
...
void CMyPlayer::Move(const XMFLOAT3& xmf3Shift, bool bUpdateVelocity)
{
	if (bUpdateVelocity)
	{
		m_xmf3Velocity = Vector3::Add(m_xmf3Velocity, xmf3Shift);
...
void CMyPlayer::Rotate(float x, float y, float z)
{
	DWORD nCurrentCameraMode = m_pCamera->GetMode();
	if ((nCurrentCameraMode == FIRST_PERSON_CAMERA) || (nCurrentCameraMode == THIRD_PERSON_CAMERA))
	{
...
{
	if (dwDirection && xRail == xNextRail)
	{
		XMFLOAT3 xmf3Shift = XMFLOAT3(0, 0, 0);
	...
	if (m_Track)
	{
		int indexNow = 5 * int(zRail);
		int indexNext = 5 * int(zNextRail);

		XMFLOAT3 resultPosition = {};
		resultPosition.y = m_xmf3Position.y;
		resultPosition.z = (1.0f - tz) * m_Track[indexNow]->GetPosition().z + tz * m_Track[indexNext]->GetPosition().z;

		if (xRail != xNextRail)
		{
			indexNow = int(xRail);
			indexNext = int(xNextRail);

			resultPosition.x = (1.0f - tx) * m_Track[indexNow]->GetPosition().x + tx * m_Track[indexNext]->GetPosition().x;
		}
		else
			resultPosition.x = m_xmf3Position.x;
...
{
	m_pBLFrame = FindFrame(L"BL");
	m_pBRFrame = FindFrame(L"BR");
	m_pFLFrame = FindFrame(L"FL");
	m_pFRFrame = FindFrame(L"FR");
}
...
void CCarPlayer::Animate(float fTimeElapsed, XMFLOAT4X4* pxmf4x4Parent)
{
	if (m_pBLFrame)
	{
		...
		m_pBLFrame->m_xmf4x4Transform = Matrix4x4::Multiply(xmmtxRotate, m_pBLFrame->m_xmf4x4Transform);
	}
	if (m_pBRFrame)
	{
		XMMATRIX xmmtxRotate = XMMatrixRotationX(XMConvertToRadians(360.0f * 4.0f) * fTimeElapsed);
		m_pBRFrame->m_xmf4x4Transform = Matrix4x4::Multiply(xmmtxRotate, m_pBRFrame->m_xmf4x4Transform);
	}

	if (m_pFLFrame)
	{
		XMMATRIX xmmtxRotate = XMMatrixRotationX(XMConvertToRadians(360.0f * 2.0f) * fTimeElapsed);
		m_pFLFrame->m_xmf4x4Transform = Matrix4x4::Multiply(xmmtxRotate, m_pFLFrame->m_xmf4x4Transform);
	}
	if (m_pFRFrame)
	{
		XMMATRIX xmmtxRotate = XMMatrixRotationX(XMConvertToRadians(360.0f * 4.0f) * fTimeElapsed);
		m_pFRFrame->m_xmf4x4Transform = Matrix4x4::Multiply(xmmtxRotate, m_pFRFrame->m_xmf4x4Transform);
	}

	CMyPlayer::Animate(fTimeElapsed, pxmf4x4Parent);
}
